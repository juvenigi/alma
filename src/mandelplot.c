...
void draw(){
  ...
  //passive-aggressively ignore custom exponent
  int (*color_alg)(double complex, int, int) = get_color;
	int ncolors = strlen(colors); // number of colors
...
void hilight_rect(int starty, int startx, int endy, int endx){
	for (int i = 0; i < height; ++i) {
		for (int j = 0; j < width; ++j) {
...
void change_var(WINDOW *win, enum CHOICE choice){
	int i;
	double d;
	char buf[MAX_INPUT];
